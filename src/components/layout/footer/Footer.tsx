...
export default function Footer({ data }: Props) {
  return (
    <footer className='w-full border-t border-t-current'>
      <div className='tl w-full pt-4 lg:pt-6 xl:pt-8 flex items-center flex-col justify-between h-full'>
        <nav className='tl px-fluid flex flex-col md:flex-row items-center justify-between flex-grow w-full gap-y-6 md:gap-y-0'>
          <FooterMenuText {...data.menuText1} />
          <FooterMenuText {...data.menuText2} />
          <FooterMenuSocial {...data.menuSocial} />
        </nav>

        <div className='px-fluid md:bg-black w-full md:mt-3'>
